...
- **已存在的記錄不刪除**：若災情檔案已建立，即使事後判斷邊界模糊，也不刪除（歷史紀錄價值）
- **下次嚴格遵循**：未來新增災情時，嚴格區分「天氣災情」與「純人為意外」
- **核心判斷**：天氣是否為災情的**主要或關鍵因素**？若只是背景條件（如雨天），不記錄；若天氣是主要原因（如大雨導致淹水、溪水暴漲），記錄
- **範例**：
  - ✅ 大雨導致淹水、坍方、溪水暴漲 → 記錄
  - ✅ 大雨後泛舟翻覆（已存在於沙德爾檔案）→ 保留（不刪除，但未來類似事件不新建）
  - ❌ 雨天車輛打滑撞山 → 不記錄
  - ❌ 雷擊意外 → 不記錄
