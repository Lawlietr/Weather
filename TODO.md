...
### 半自動方案（建議採行；2026/8/30 與 AG 討論後定案）

> **背景與政策定位**：AGENTS.md 核心原則為「災情新聞仍需人工把關，**不自動推 RSS**」。因此本方案**不做全自動接入**，僅在 build 階段自動抓 RSS 產出「候選清單」，由人 / LLM 審查後再寫進事件 markdown 的「災情新聞來源」區塊。
...
```
build 自動抓 RSS → 去重 + 標好結構 → 產出「候選清單」(rss_candidates.json)
  → 你 / LLM 只審查：哪些跟當前這件颱風 / 災情有關
  → 挑出來的貼進事件 markdown 的「災情新聞來源」區塊
  → build 再部署（deploy.sh 不動）
```

#### 技術可行性（已實測 2026/8/30）

- 8 家 verified feed 中 7 家可自動抓下並解析（中央社 20、自由時報 40、台視 40、新頭殼 100、關鍵評論 18、民報 10、報導者 10；公視 atom 需 namespace 修正）。
- 抓取模式沿用 `build.sh` 的 curl + 重試 + 失敗降級設計；單一來源 404/超時跳過記 warning。
- 新聞渲染直接reuse 現有 `extract_news_by_county()` 結構，不需另建顯示層。

#### 與全自動方案的取捨

| | 全自動（§1 原始寫法） | 半自動（本方案，建議） |
|---|---|---|
| RSS 抓取 | 自動 | 自動 |
| 相關性過濾 | 程式自動 | 程式初判 + 人工覆核 |
| 上線 | 程式直接推 | 人工挑後寫 markdown 再推 |
| 符合「人工把關」 | ❌ 違反 | ✅ 符合 |
| 適合情境 | 需即時、可接受風險 | 省勞動但守把關（本專案現況） |
