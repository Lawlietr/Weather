...
2. `build/cbph.py`：抓 4 類→驗證→合併進 `map.geo.json` — **⏳ 進行中（2026/9/1）**：抓取＋驗證＋GeoJSON Feature 已實作（`build/cbph.py`；屏東縣單縣市採集測試通過：歷史池 40 筆、40/40 含官方 polygon、UTC→UTC+8、503 容錯均驗證）。剩：合併進 `map.geo.json`＋掛 build 流水線（隨步驟 3 `/map/` 頁一起做）。注意：採集是全縣一次抓（`/api/global/` 不區縣），無逐縣市工作項——「單縣市採集」只是實作期的驗證方式。
