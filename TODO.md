...
> 最後更新：2026/9/13（同步 main §7 颱風動態淘汰；2026/9/7 全檔精簡：已完成項目細節收為一線，細節看 git history／模組註解）

> 優先級：**§7 颱風動態淘汰機制（最終高優先級，待實作）**、§2 地圖紅警（高，實作中：執行順序 1–3 ✅、下一步 4 觀測層）＞ §6 停班停課板塊（中高，待實作）＞ §4 平常天氣報導（中，構想待討論）＞ §1 RSS 殘細項（低，剩隨事件微調）＞ §3 ja 移除／§5 分享按鈕（低）。

---

## 7. 颱風動態區塊淘汰／過濾機制（**最終高優先級，2026/9/13 提出；暂不實作**）
...
**對照現有機制**：警報卡 `render_alert_card()` 已設 `LIFTED_TTL_HOURS = 48`，會淘汰「已解除」且超過 48h 的警報；**颱風卡沒有對等的淘汰機制**，這是設計缺口。
...
在 `build/cwa.py: render_typhoon_card()` 加入淘汰機制，二選一或並用：

- **A｜新鮮度 TTL（推薦先做）**：若氣旋「最新一筆 analysis fix」距今超過某時限（例如 >24h 未更新，代表 CWA 不再追蹤／已消散），從卡片移除；全部移除則整張卡回傳現成「無活動中熱帶氣旋」空狀態。對照 `LIFTED_TTL_HOURS` 寫法，工作量小、風險低。
- **B｜對台相關性過濾**：依 AGENTS.md「颱風檔案建立門檻」精神，只保留可能影響台灣的氣旋（需定義判準，較主觀，可日後再補）。

### 執行順序（實作時）

1. 在 `cwa.py` 新增常數（如 `TYphoon_STALE_HOURS`）與過濾邏輯，重構 `fetch_typhoons()` 或 `render_typhoon_card()`。
2. 全部淘汰時的空狀態文案走現有 i18n `typhoon_none`／`typhoon_nodata`。
3. i18n：若新增文案則補 `build/i18n.py`。
4. 文件同步：`AGENTS.md`「網站專案」或 `WORKFLOW.md` 補此過濾規則；本節標記已實作。
5. 在 DEV 分支開發、經確認後合併 main，再 build＋手動部署。

### 與現有架構的兼容性

- 不影響 `颱風/`、`災情/` 的事件驅動流程；只改首頁 CWA 區塊渲染。
- 掛在現有 build 流水線，不需新排程、新金鑰。
- 與 §2 地圖紅警的 W-C0034-005 颱風軌跡/風圈層互斥依賴（地圖層要完整軌跡；首頁卡片要過濾後相關氣旋），實作時注意兩處取用資料應分開（地圖用原始、首頁用過濾後）。
