...
**實作**：`build/dgpa.py`（feed 結構、陷阱、`is_current()` 判斷規則全在該檔頭註；欄位細節另見 `AGENTS.md`「DGPA 停班停課 feed」節）。首頁置位（2026/9/15 使用者定案）：
...
**A/B 並用，判斷標準＝對應事件 status**（使用者已認可，規則與最小事件檔範本已寫入 `AGENTS.md`「零星災情的寫法」節）：
