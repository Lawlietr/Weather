...
> 最後更新：2026/9/13（新增 §9 氣象總覽卡片排序：無颱風時颱風卡置底，已實作＋測試站驗證、待合併 main；§8 cbph 503 假警報已實作＋測試站 wea-testing 驗證、待合併 main；§7 颱風動態淘汰已合併 main 上線；另設測試站 wea-testing／weatesting.avpclub.eu.org，詳 WORKFLOW.md §6；2026/9/7 全檔精簡：已完成項目細節收為一線，細節看 git history／模組註解）

> 優先級：§9 氣象總覽卡片排序（高，✅ 已實作、測試站驗證、待合併 main）≈ §8 cbph 503 假警報（高，✅ 已實作、已合併 main、待自動部署生效）＞ §2 地圖紅警（高，實作中：執行順序 1–3 ✅、下一步 4 觀測層）＞ §6 停班停課板塊（中高，待實作）＞ §4 平常天氣報導（中，構想待討論）＞ §10 零星災情縣市分組寫法（中低，待選定 A/B）＞ §1 RSS 殘細項（低，隨隨事件微調）＞ §3 ja 移除／§5 分享按鈕（低）。

---

## 9. 氣象總覽卡片排序：無颱風時颱風卡置底（高優先級；2026/9/13 定案並實作、測試站已驗證，待合併 main）
...
實作：`build/cwa.py: cwa_section_html()` 約 10 行（`typhoon_first = bool(typhoons_live) or bool(stale.get("typhoons"))` 決定插入位置）。單元測試 5 案（有氣旋/空/空＋stale/有氣旋＋stale/過時淘汰）＋真實 build 驗證（2026/9/13：警報→雨量→颱風空卡）。已部署測試站 wea-testing 驗證。**待合併 main**。
...
## 10. 零星災情的縣市分組寫法（🟡🟢 中低優先級；2026/9/14 提出，待選定 A/B 後實作）
...
| 方案 | 做法 | 適用 | 影響範圍 |
|------|------|------|----------|
| **A　併入既有事件檔** | 把零星災情以四欄表格寫入現有事件檔的縣市章節（如 `## 宜蘭縣災情`） | 該災情屬於某個**仍活躍/未封存**事件的延續 | 動既有的 `災情/2026/09/0904_低壓帶_南北台灣豪雨.md` 等檔；`最後修改` 行原地更新 |
| **B　建最小事件檔** | 新建 `災情/{YYYY}/{MM}/{MMDD}_{事件}_{名稱}.md`，含最小 front matter（`event`／`status: active∣ended`／`severity: 🟢一般`）＋災情表 | 獨立新事件、無既有檔可併 | 新增檔案，走既有 build 流水線，build 仍按縣市聚合 |

**兩者 build 後效果相同**（都會進首頁「各縣市災情」並依縣聚合）。差異在檔案組織：A 節省檔案數但會改動既有事件檔；B 檔案獨立、易追蹤但事件清單會變長。

### 待辦

1. 由使用者決定採用 A 或 B（或兩者並用：延續事件用 A、全新事件用 B）。
2. 若採 B，考慮在 `AGENTS.md`「災情檔案」增一段「最小事件檔範本」（front matter 最小集＋災情表格式）。
3. 實務上可能不需動 build 程式；若日後要「某縣市歷年全量災情」獨立頁面，才需改 `site.py` 新增彙整頁。
